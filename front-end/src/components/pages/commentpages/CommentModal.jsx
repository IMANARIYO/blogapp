...
const CommentModal = ({ show, handleClose, postId, onCommentAdded }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await api.post(`/posts/${postId}/comments`, { content: commentContent, authorId: 1 }); // Replace `1` with the actual user ID
  ...
  return (
    <div className={`modal fade ${show ? 'show d-block' : ''}`} tabindex="-1" aria-labelledby="addCommentModalLabel" aria-hidden="true">
